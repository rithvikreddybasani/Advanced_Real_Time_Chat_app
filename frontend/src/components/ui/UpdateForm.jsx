...
const UpdateForm = ({ onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (localStorage.getItem("formdata")) {
      localStorage.removeItem("formdata"); // just
    }
    ...
    >
      ...
      <div className="relative">
        <TextField
          ...
          onChange={handleChange}
          variant={`after:content-['*'] after:ml-1 after:text-red-500`}
          {...signupFields.confirmPassword} // last case replace chey ta ante you
        />
        <div
          className="absolute z-100 right-3 top-[60%] text-xl cursor-pointer"
        ...
        >
          {" "}
          <Icon
            logic={showConfirmPassword ? "#icon-eye" : "#icon-eye-hidden"}
          />
        </div>
      </div>
